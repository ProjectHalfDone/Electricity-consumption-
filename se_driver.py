...
#           Import data: SE data
#------------------------------------------------------
se = pd.read_csv('C:/SE data/fra Emil DE/clean_under_200_low_w1_high_w2_data_2_use.csv',parse_dates=['date'], index_col='date')

#se = pd.read_csv('C:/SE data/esbjerg_rdy_sorted.csv', sep=',', parse_dates=['dato'], dtype={'forbrug':np.float64})#, nrows = 8000000)
#se = pd.read_csv('C:/SE data/esbjerg_rdy.csv', sep=';', parse_dates=['dato'], decimal = ',')#, nrows = 3000)
#se = pd.read_csv('C:/SE data/esbjerg_rdy.csv', sep=';', parse_dates=['dato'], index_col='dato', decimal = ',', nrows = 3000)
#Check that AVA january data is imported correct
print('Shape of january, expect (168, 32241) obs',se.shape)
...
def acf_prep(data, lags = 24, correction='Yes'):
    ...
    return aacf
    #aacf = acf_prep(se, lags = 24, correction='Yes')
